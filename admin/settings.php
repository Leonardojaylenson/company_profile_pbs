...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['hero_video']['name'])) {
        ...
        if ($path) {
            $stmt = $pdo->prepare("INSERT INTO settings (setting_key,setting_value) VALUES (?,?) ON DUPLICATE KEY UPDATE setting_value=?");
            $stmt->execute(['hero_video', $path, $path]);
        }
    }
    logActivity($admin['id'], 'UPDATE_SETTINGS', 'Memperbarui pengaturan situs');
        ...
        <div class="adm-content">
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <ul class="nav nav-tabs mb-4" id="settingsTabs">
                    ...
                    <li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab-about">Tentang</button></li>
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane fade show active" id="tab-general">
                        <div class="adm-card">
                            ...
                            <div class="adm-card-body">
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        <label class="adm-form-label">Nama Situs</label>
                                        <input type="text" name="site_name" class="adm-input" value="<?= htmlspecialchars($s['site_name'] ?? '') ?>">
                                    </div>
                                    <div class="col-md-6">
                                        <label class="adm-form-label">Tagline</label>
                                        <input type="text" name="site_tagline" class="adm-input" value="<?= htmlspecialchars($s['site_tagline'] ?? '') ?>">
                                    </div>
                                    <div class="col-12">
                                        <label class="adm-form-label">Deskripsi Situs</label>
                                        <textarea name="site_description" class="adm-input"><?= htmlspecialchars($s['site_description'] ?? '') ?></textarea>
                                    </div>
                                    <div class="col-12">
                                        <label class="adm-form-label">Meta Keywords</label>
                                        <input type="text" name="meta_keywords" class="adm-input" value="<?= htmlspecialchars($s['meta_keywords'] ?? '') ?>" placeholder="pengiriman laut, ekspedisi, batam">
                                    </div>
                                    <div class="col-md-4">
                                        <label class="adm-form-label">Logo</label>
                                        <?php if (!empty($s['logo'])): ?>
                                        <img src="<?= assetUrl($s['logo']) ?>" class="img-preview d-block mb-2">
                                        <?php endif; ?>
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <img src="<?= assetUrl($s['favicon']) ?>" class="img-preview d-block mb-2">
                                        <?php endif; ?>
                                        <input type="file" name="favicon" class="adm-input" accept="image/*">
                                    </div>
                                    <div class="col-md-4">
                                        <label class="adm-form-label">OG Image (Share Preview)</label>
                                        <?php if (!empty($s['og_image'])): ?>
                                        <img src="<?= assetUrl($s['og_image']) ?>" class="img-preview d-block mb-2">
                                        <?php endif; ?>
                                        <input type="file" name="og_image" class="adm-input" accept="image/*">
                                    </div>
                                    <div class="col-12">
                                        <label class="adm-form-label">Copyright Footer</label>
                                        <input type="text" name="footer_copyright" class="adm-input" value="<?= htmlspecialchars($s['footer_copyright'] ?? '© 2025 PT. Prima Bahari Sejahtera.') ?>">
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="tab-pane fade" id="tab-hero">
                        <div class="adm-card">
                            ...
                            <div class="adm-card-body">
                                <div class="row g-3">
                                    <div class="col-12">
                                        <label class="adm-form-label">Judul Utama Hero</label>
                                        <input type="text" name="hero_title" class="adm-input" value="<?= htmlspecialchars($s['hero_title'] ?? 'Solusi Pengiriman Laut Terpercaya') ?>">
                                    </div>
                                    <div class="col-12">
                                        <label class="adm-form-label">Sub-judul Hero</label>
                                        <textarea name="hero_subtitle" class="adm-input" rows="3"><?= htmlspecialchars($s['hero_subtitle'] ?? '') ?></textarea>
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <p class="adm-form-hint mb-1">Video aktif: <?= basename($s['hero_video']) ?></p>
                                        <?php endif; ?>
                                        <input type="file" name="hero_video" class="adm-input" accept="video/mp4,video/webm">
                                        <p class="adm-form-hint">MP4/WebM, maks 50MB. Jika kosong, tampil background animasi.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="tab-pane fade" id="tab-about">
                        <div class="adm-card">
                            ...
                            <div class="adm-card-body">
                                <div class="row g-3">
                                    <div class="col-12">
                                        <label class="adm-form-label">Judul Tentang</label>
                                        <input type="text" name="about_title" class="adm-input" value="<?= htmlspecialchars($s['about_title'] ?? 'Pengiriman Andal, Koneksi Nusantara') ?>">
                                    </div>
                                    <div class="col-12">
                                        <label class="adm-form-label">Deskripsi Tentang</label>
                                        <textarea name="about_description" class="adm-input" rows="5"><?= htmlspecialchars($s['about_description'] ?? '') ?></textarea>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="adm-form-label">Visi</label>
                                        <textarea name="vision" class="adm-input" rows="4"><?= htmlspecialchars($s['vision'] ?? '') ?></textarea>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="adm-form-label">Misi</label>
                                        <textarea name="mission" class="adm-input" rows="4"><?= htmlspecialchars($s['mission'] ?? '') ?></textarea>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="adm-form-label">Foto Tentang Kami</label>
                                        <?php if (!empty($s['about_image'])): ?>
                                        <img src="<?= uploadUrl($s['about_image']) ?>" class="img-preview d-block mb-2">
                                        <?php endif; ?>
                                        <input type="file" name="about_image" class="adm-input" accept="image/*">
                                    </div>
                                    <div class="col-12">
                                        <label class="adm-form-label">Judul CTA Section</label>
                                        <input type="text" name="cta_title" class="adm-input" value="<?= htmlspecialchars($s['cta_title'] ?? 'Siap Kirimkan Muatan Anda?') ?>">
                                    </div>
                                    <div class="col-12">
                                        <label class="adm-form-label">Deskripsi CTA</label>
                                        <textarea name="cta_description" class="adm-input" rows="2"><?= htmlspecialchars($s['cta_description'] ?? '') ?></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="tab-pane fade" id="tab-social">
                        <div class="adm-card">
                            ...
                            <div class="adm-card-body">
                                <div class="row g-3">
                                    <div class="col-md-6"><label class="adm-form-label"><i class="bi bi-instagram me-1"></i>Instagram URL</label><input type="url" name="social_instagram" class="adm-input" value="<?= htmlspecialchars($s['social_instagram'] ?? '') ?>" placeholder="https://instagram.com/..."></div>
