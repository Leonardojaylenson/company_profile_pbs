...
if (isset($_GET['delete'])) {
    ...
    $row = $stmt->fetch();
    $pdo->prepare("DELETE FROM routes WHERE id=?")->execute([(int)$_GET['delete']]);
    logActivity($admin['id'], 'DELETE_ROUTE', 'Menghapus rute', [
        'Rute' => ($row['origin']??'?').' → '.($row['destination']??'?'),
        'ID'   => (string)(int)$_GET['delete'],
    ]);
    header('Location: ' . BASE_URL . '/admin/routes.php'); exit;
}
if (isset($_GET['toggle'])) {
    $stmt = $pdo->prepare("SELECT origin, destination, is_active FROM routes WHERE id=?");
    $stmt->execute([(int)$_GET['toggle']]);
    $row = $stmt->fetch();
    $pdo->prepare("UPDATE routes SET is_active=1-is_active WHERE id=?")->execute([(int)$_GET['toggle']]);
    logActivity($admin['id'], 'TOGGLE_ROUTE', 'Mengubah status rute', [
        'Rute'   => ($row['origin']??'?').' → '.($row['destination']??'?'),
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($origin)||empty($destination)) { $error = 'Asal dan tujuan wajib diisi.'; }
        ...
        if ($id > 0) {
            $old = $pdo->prepare("SELECT * FROM routes WHERE id=?");
            $old->execute([$id]);
            $old = $old->fetch();
            ...
            $changes = [];
            if ($old['origin']      !== $origin)      $changes['Asal']     = ['old'=>$old['origin'],     'new'=>$origin];
            if ($old['destination'] !== $destination)  $changes['Tujuan']   = ['old'=>$old['destination'],'new'=>$destination];
            if ($old['duration']    !== $duration)     $changes['Durasi']   = ['old'=>$old['duration'],   'new'=>$duration];
            if ($old['frequency']   !== $frequency)    $changes['Jadwal']   = ['old'=>$old['frequency'],  'new'=>$frequency];
            if ($old['notes']       !== $notes)        $changes['Catatan']  = ['old'=>$old['notes'],      'new'=>$notes];
            if ((int)$old['sort_order'] !== $sort_order) $changes['Urutan'] = ['old'=>(string)$old['sort_order'],'new'=>(string)$sort_order];
            if ((int)$old['is_active']  !== $is_active)  $changes['Status'] = ['old'=>$old['is_active']?'Aktif':'Nonaktif','new'=>$is_active?'Aktif':'Nonaktif'];

            logActivity($admin['id'], 'EDIT_ROUTE', 'Edit rute: '.$origin.' → '.$destination.(!empty($changes)?' ('.count($changes).' perubahan)':''), $changes);
        } else {
            ...
            logActivity($admin['id'], 'ADD_ROUTE', 'Menambahkan rute baru', [
...
<div class="adm-layout">
    ...
    <div class="adm-main">
        <div class="adm-topbar">
            ...
            <div class="topbar-actions"><button class="topbar-btn btn-primary-adm" onclick="openModal()"><i class="bi bi-plus-lg me-1"></i>Tambah Rute</button></div>
        </div>
        <div class="adm-content">
            ...
            <div class="adm-card">
                ...
                <?php if (empty($routes)): ?>
                <div class="empty-state"><i class="bi bi-map"></i><p>Belum ada rute.</p></div>
                <?php else: ?>
                <div class="table-responsive">
                    <table class="adm-table">
                        ...
                        <tr>
                            <td class="text-muted nowrap"><?= $i+1 ?></td>
                            <td class="nowrap">
                                ...
                                <?php if ($r['notes']): ?><small class="text-muted d-block mt-1"><?= htmlspecialchars($r['notes']) ?></small><?php endif; ?>
                            </td>
                            <td class="nowrap"><?= htmlspecialchars($r['duration']) ?></td>
                            <td class="nowrap"><?= htmlspecialchars($r['frequency']) ?></td>
                            <td class="nowrap"><a href="?toggle=<?= $r['id'] ?>" class="adm-badge <?= $r['is_active']?'green':'gray' ?>"><?= $r['is_active']?'Aktif':'Nonaktif' ?></a></td>
                            
                            <td class="nowrap">
                                <div class="action-btns">
                                    <a href="#" class="btn-adm edit" onclick="openModal(<?= htmlspecialchars(json_encode($r)) ?>);return false;"><i class="bi bi-pencil"></i></a>
                                    <a href="?delete=<?= $r['id'] ?>" class="btn-adm del" onclick="return confirm('Hapus rute ini?')"><i class="bi bi-trash"></i></a>
                                </div>
                            </td>
                        </tr>
                        <?php endforeach; ?>
...
<div class="adm-modal">
    <div class="adm-modal-header"><h5 id="modalTitle">Tambah Rute</h5><button class="adm-modal-close" onclick="closeModal()"><i class="bi bi-x"></i></button></div>
    <form method="POST">
    ...
    <div class="adm-modal-body">
        <div class="row g-3">
            ...
            <div class="col-md-3 d-flex align-items-end pb-1"><div class="form-check"><input type="checkbox" name="is_active" id="f_is_active" class="form-check-input" checked><label class="form-check-label" for="f_is_active">Aktif</label></div></div>
